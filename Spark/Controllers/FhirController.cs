...
namespace Spark.Controllers
{
    ...
    public class FhirController : ApiController
    {
        ...
        public FhirResponse Create(string type, Resource resource)
        {
            ...
            if (!Validate(resource, out errors))
            {
                OperationOutcome outcome;

                SetOutcome(errors, out outcome);
                
                return new FhirResponse(HttpStatusCode.BadRequest, outcome);
            }
            #endregion
        ...
        */
        ...
        #region validation
        private bool Validate(Resource resource, out IList<ValidationError> errors)
        {
            JSchema schema;

            GetSchema(out schema);
           
            string resourceJSON = FhirSerializer.SerializeResourceToJson(resource);
            JObject jObject = JObject.Parse(resourceJSON);

            return jObject.IsValid(schema, out errors);
        }               

        private static void GetSchema(out JSchema schema)
        {
            using (StreamReader file = new StreamReader(LoadResourse()))
        ...
        private static Stream LoadResourse()
        {
            return typeof(FhirController).Assembly.GetManifestResourceStream("Spark.validation.json");
        }

        private static void SetOutcome(IList<ValidationError> errors, out OperationOutcome outcome)
        {
